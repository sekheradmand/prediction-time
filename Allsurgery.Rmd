...
x<-read.csv("D:\\3\\datamining\\surgery time\\sepration\\New folder\\dataclean _numeric.csv", header = TRUE,fileEncoding = "UTF-8")#"UCS-2LE")
#View(x)
y<-x
...
ml_all = lm( anesthesia.time~ gender +data.classs +surgeon+k.anesthesia+age+ k.s+surgery.code, data = train_set)
summary( ml_all )
plot(ml_all )
...
plot(ml_predict, actual_test,main="All surgery",
     xlab = "Predicted Values",
     ylab = "Observed Values")
abline(a = 0, b = 1, lwd=2,
       col = "blue")
#RMSE plot
ggplot(y$anesthesia.time.Length, aes(x = ml_predict, y = actual_test)) + 
  geom_pointrange(aes(ymin = 0, ymax = actual_test)) + 
  geom_hline(yintercept = 0, linetype = 3) + 
  ggtitle("prediction vs. observed")
```
...
test_set =part_data $ part2
#////////////////////////////////////////////////////////////////////
fitreg<-rpart(anesthesia.time ~gender +data.classs +surgeon+k.anesthesia+ anesthesia+age+ k.s+surgery.code ,method="anova",data=train_set)
...
actual_test  = test_set $anesthesia.time 
#head(test_set)
modelsvm = svm( anesthesia.time~ gender +data.classs +surgeon+k.anesthesia+age+ k.s+surgery.code, data = y)
...
train = y[indexes, ]
test = y[-indexes, ]
train_setx = data.matrix(train[, -9])
train_sety = train[,9]

test_setx = data.matrix(test[, -9])
test_sety = test[, 9]
...
plot(x, test_sety, main="All surgery",col = "red", type = "l")
